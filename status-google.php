...
?>
<form action="add_data_google.php" method="POST">
...
<table>
    ...
    $sql = mysqli_query($db_connection, "SELECT * FROM status WHERE email = '$email' ORDER BY time ASC");
    if (mysqli_num_rows($sql) > 0) {
        ...
        while ($user = mysqli_fetch_assoc($sql)) {
            ...
            echo "<td><a href='edit_data_google.php?id=" . $user['id'] . "'>แก้ไข</a></td>";
            echo "<td><a href='delete_data_google.php?id=" . $user['id'] . "' onclick='return confirm(\"คุณต้องการลบข้อมูลนี้หรือไม่?\")'>ลบ</a></td>";
            echo "</tr>";
            $order++;
            $total += $user['number'];
        }
