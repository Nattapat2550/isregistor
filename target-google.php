...
?>
<form action="add_datatarget_google.php" method="POST">
<a class="head">Target Money</a><br>
...
<table>
    ...
    $sql = mysqli_query($db_connection, "SELECT * FROM target WHERE email = '$email' ORDER BY time ASC");
    if (mysqli_num_rows($sql) > 0) {
        ...
        while ($user = mysqli_fetch_assoc($sql)) {
            ...
            if ($user['number'] > 0) {
                ...
                echo "<td><a href='edit_datatarget_google.php?id=" . $user['id'] . "'>แก้ไข</a></td>";
                echo "<td><a href='minus_data_google.php?id=" . $user['id'] . "'>ผ่อน</a></td>";
            } else {
            ...
            echo "<td><a href='delete_datatarget_google.php?id=" . $user['id'] . "' onclick='return confirm(\"คุณต้องการลบข้อมูลนี้หรือไม่?\")'>ลบ</a></td>";
            echo "</tr>";
            $order++;
            $total += $user['number'];
        }
