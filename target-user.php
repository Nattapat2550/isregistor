...
<form action="add_datatarget.php" method="POST">
...
<table>
    ...
    try {
        ...
        if (isset($row['email'])) {
            $email = $row['email'];
            $sql = "SELECT * FROM target WHERE email = :email ORDER BY time ASC";
            ...
            if ($stmt->rowCount() > 0) {
                foreach ($result as $row) {
                    ...
                    if ($row['number'] > 0) {
                        ...
                        echo "<td>" . $row['time'] . "</td>";
                        echo "<td><a href='edit_datatarget.php?id=" . $row['id'] . "'>แก้ไข</a></td>";
                        echo "<td><a href='minus_data.php?id=" . $row['id'] . "'>ผ่อน</a></td>";
                    } else {
                    ...
                    echo "<td><a href='delete_datatarget.php?id=" . $row['id'] . "' onclick='return confirm(\"คุณต้องการลบข้อมูลนี้หรือไม่?\")'>ลบ</a></td>";
                    echo "</tr>";
                    $order++;
                    $total += $row['number'];
                }
